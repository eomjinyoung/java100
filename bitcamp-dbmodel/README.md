...
### 3) PK로 선정되지 않은 나머지 키는?
- 나머지 후보 키들은 유니크 컬럼으로 지정하여 
    데이터가 중복되지 않도록 한다.

### 4) 테이블 간의 관계 지정
- 테이블 사이의 1:다, 다:다 등의 관계를 지정한다.
- 1쪽이 부모 테이블이고 다 쪽이 자식테이블이다.
- 다 대 다 관계를 DBMS에서 물리적으로 지원하지 않기 때문에
  물리모델 과정에서 일 대 다의 관계로 변경해야 한다.
- 일 대 다의 관계에서 다 쪽 테이블은 일 쪽 테이블의 PK를 참조한다.
  그래서 다쪽 테이블에 일쪽 테이블의 PK 컬럼을 값을 갖는 컬럼이 추가된다.
  그 컬럼을 외부키(foreign key)라 부른다.
...
### 테이블 간 관계
- 테이블의 데이터끼리 관계가 있는 경우 서로 연결하여 부모-자식 관계를 맺는다.
...
- 보통 PK를 선정하기 전에 후보키들이 유니크 컬럼으로 지정된다.
...
### 15) 포워드 엔지니어링(forward engineering)
- 모델 ----> 코드
